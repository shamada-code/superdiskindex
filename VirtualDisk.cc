...
void VirtualDisk::AddSector(u8 c, u8 h, u8 s, u8 r, void *p, u32 size, bool crc1ok, bool crc2ok)
{
	...
	if ((Disk.Cyls[c].Heads[h].Sectors[s].Revs[r].crc1ok)&&(!crc1ok)) { clog(2, "# Sector %02d/%01d/%02d already read in better quality. skipping.\n",c,h,s); return; }
	if ((Disk.Cyls[c].Heads[h].Sectors[s].Revs[r].crc2ok)&&(!crc2ok)) { clog(2, "# Sector %02d/%01d/%02d already read in better quality. skipping.\n",c,h,s); return; }
